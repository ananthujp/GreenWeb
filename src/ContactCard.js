...
function ContactCard({ name, role, img, i, insta, linkd, fb, email }) {
  ...
  return (
    ...
    >
      <div className="transition-all bg-purple-100 hover:bg-purple-300 flex justify-end rounded-full  border-8 lg:w-36 md:w-32 w-24 lg:h-36 md:h-32 h-24 border-white shadow-md border-double">
        <img
          className="-mt-12 rounded-b-full transition-all duration-500 lg:w-36 md:w-32 w-24 lg:h-44 md:h-40 h-32   object-cover"
          style={{
      ...
      <div className="flex justify-between w-full mt-2 text-gray-400 stroke-current ">
        {[insta, linkd, fb, email].map(
          (data, index) =>
            data && (
              <div
                className="hover:text-fuchsia-200 transition-all"
                onClick={
                  index === 3
...
export const ContactCard2 = ({
  ...
  return (
    <motion.div
      ...
      // style={{padding:'1rem',border: '2px solid ',borderColor: '#f3e8ff',backgroundColor:'#faf5ff',...hover && {borderColor: '#e9d5ff',backgroundColor: '#f3e8ff'}}}
    >
      <div className="transition-all bg-purple-100 hover:bg-purple-300 flex justify-end rounded-md  border-8 lg:w-36 md:w-32 w-24 lg:h-36 md:h-32 h-24 border-white shadow-md border-double">
        <img
          className=" rounded-md transition-all duration-500 lg:w-36 md:w-32 w-24 lg:h-32 md:h-28 h-20   object-cover"
          style={{
            filter: "grayscale(100%)",
            ...(hover && { filter: "grayscale(0%)" }),
          }}
