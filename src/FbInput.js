...
function FbInput({ img, name, batch }) {
  ...
  const handleLogin = () => {
    batchrole.batch
      ? signInWithPopup(auth, provider).then(
          (response) => console.log(response.uid),
          (err) => console.log(err)
        )
      : setBatchHint(
          "ring-2 transition-all ring-red-600 rounded-md mt-4 text-red-600 animate-pulse"
        );
  };
  const checkLogin = () =>
    batchrole && batchrole.batch != null ? null : dispatch(logout());
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="md:w-1/3 w-full h-128"
    >
      <div className="px-3 md:px-4 flex-none my-4 w-full">
        {user && user.email && batchrole && batchrole.batch ? (
          <figure className="shadow-lg rounded-xl flex-none md:w-xl">
            <figcaption className="flex items-center space-x-4 p-6 md:px-10 md:py-6 bg-gradient-to-br rounded-t-xl leading-6 font-semibold text-white from-purple-500 to-indigo-500">
              ...
              <div className="flex-auto">
                {user.displayName}
                <br />
                <span className="text-purple-100">{batchrole.batch}</span>
              </div>
              <cite className="flex">
                <div
            ...
            <blockquote className="rounded-b-xl bg-white px-6 py-4 md:p-6 text-lg md:text-xl leading-8 md:leading-8 font-semibold text-gray-900">
              <svg
                width="45"
                height="36"
                className="mb-5 fill-current text-purple-100"
              >
                <path d="M13.415.001C6.07 5.185.887 13.681.887 23.041c0 7.632 4.608 12.096 9.936 12.096 5.04 0 8.784-4.032 8.784-8.784 0-4.752-3.312-8.208-7.632-8.208-.864 0-2.016.144-2.304.288.72-4.896 5.328-10.656 9.936-13.536L13.415.001zm24.768 0c-7.2 5.184-12.384 13.68-12.384 23.04 0 7.632 4.608 12.096 9.936 12.096 4.896 0 8.784-4.032 8.784-8.784 0-4.752-3.456-8.208-7.776-8.208-.864 0-1.872.144-2.16.288.72-4.896 5.184-10.656 9.792-13.536L38.183.001z"></path>
              </svg>
              <textarea
                id="fbInputMsg"
                className="w-full placeholder-gray-400 outline-none focus:bg-purple-50 rounded-xl p-2 text-gray-600"
              ...
              <div className="w-full flex border-t border-gray-100">
                <button
                  onClick={handleWrite}
                  class="mt-4 my-auto ml-auto px-6 py-3 text-sm transition-all text-white bg-purple-600 font-semibold rounded-3xl border border-purple-200 hover:text-white hover:bg-purple-800 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2"
                >
                  Submit
                </button>
              </div>
          ...
          <figure className="shadow-lg rounded-xl flex-none md:w-xl">
            ...
            <blockquote className="rounded-b-xl bg-white px-6 py-4 md:p-6 text-base md:text-lg leading-8 md:leading-8 font-semibold text-gray-900">
              <div className="mb-5 fill-current text-gray-400 font-thin flex flex-row text-left text-base">
                {" "}
                Start posting feedbacks by selecting your role followed by login
                with your Google ID.{" "}
              </div>

              <div className="w-full items-center flex border-t border-gray-100">
                <div className={batchHint}>
                  <Dropdown />
                </div>
                <button
                  onClick={handleLogin}
                  class="mt-4 flex flex-row my-auto mx-auto px-6 py-2 text-sm items-center transition-all text-gray-600 bg-gray-50 font-semibold rounded-3xl border border-purple-200 hover:text-gray-800 hover:bg-gray-200 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2"
                >
                  <GIcon />
                  &nbsp; 2. Login
                </button>
